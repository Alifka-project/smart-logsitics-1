  ...
  const handleSendSMS = async (): Promise<void> => {
    try {
      ...
      const deliveryId = String(
        delivery.id || (delivery as Record<string, unknown>)['ID'] || '',
      ).trim();
      if (!deliveryId) {
      ...
      const msg = e.response?.data?.message ?? e.message ?? 'Failed to send SMS';
      setResult({ ok: false, error: msg, ...(e.response?.data ?? {}) });
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          {!result ? (
            <>
              ...
              <div>
                ...
                <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 p-3 rounded-lg text-sm text-gray-700 dark:text-gray-200 space-y-2">
                  ...
                  <p>
                    Your Electrolux order
                    {delivery.poNumber ? ` #${delivery.poNumber}` : ''} is ready for delivery.
                  </p>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex-1 px-4 py-2 bg-blue-800 hover:bg-blue-900 disabled:bg-gray-400 text-white rounded-lg transition-colors font-semibold flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
            ...
            <>
                    ...
                    {(result.confirmationLink || result.token) && (
                      <div className="border-t border-green-200 dark:border-green-800 pt-3">
                        <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-2 flex items-center gap-1">
                          <Link2 className="w-3 h-3" /> Confirmation Link (copy and share):
                        </p>
                        <input
                          type="text"
                          value={
              ...
              <button
                onClick={onClose}
                className="w-full px-4 py-2 bg-blue-800 hover:bg-blue-900 text-white rounded-lg transition-colors font-semibold"
              >
                Done
              </button>
            </>
            ...
            <>
              <div className="text-center py-4">
                ...
                {result.d7Detail && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 break-all font-mono bg-gray-100 dark:bg-gray-800 p-2 rounded">
                    {result.d7Detail}
                  </p>
                )}
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={onClose}
                  className="flex-1 px-4 py-2 bg-blue-800 hover:bg-blue-900 text-white rounded-lg transition-colors font-semibold"
                >
                  Close
                </button>
              </div>
