...
interface SearchResult { id: string; customer?: string; address?: string; status?: string }
interface SearchDriver { id: string; fullName?: string; username?: string; email?: string; phone?: string; active?: boolean }
interface SearchResults {
  ...
  insights?: string[];
  insightOnly?: boolean;
  kpis?: { leader?: string; leaderCount?: number };
  insight?: { type?: string; chartData?: Array<Record<string, unknown>>; period?: string };
  chart?: { data?: Array<Record<string, unknown>>; period?: string };
}
...
interface AISearchBarProps {
  ...
  triggerSuggestion: (text: string) => void;
  handleResultClick: (result: SearchResult | SearchDriver, type: string) => void;
  clearSearch: () => void;
  theme: string;
  flex?: string;
  maxWidth?: string;
...
const AISearchBar = memo(function AISearchBar({
  outerRef, inputRef, searchQuery, setSearchQuery, searchLoading,
  searchResults, showSearch, setShowSearch, handleSearch, triggerSuggestion,
  handleResultClick, clearSearch, theme, flex, maxWidth,
}: AISearchBarProps) {
  ...
  return (
    <div ref={outerRef} style={{ position: 'relative', flex: flex || '1 1 280px', maxWidth: maxWidth || '420px', minWidth: '140px' }}>
      <form onSubmit={handleSearch}>
        <div style={{ display:'flex', alignItems:'center', gap:'7px', background: theme==='dark' ? 'var(--surface2)' : '#f0f1f8', border:`1px solid ${showSearch||searchQuery ? 'var(--primary)' : 'var(--border)'}`, borderRadius:'999px', padding:'6px 11px 6px 10px', transition:'border-color 0.2s, box-shadow 0.2s', boxShadow: showSearch||searchQuery ? '0 0 0 3px var(--primary-glow)' : 'none' }}>
          {searchLoading
          ...
          <input ref={inputRef} type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={() => setShowSearch(true)} placeholder="AI Search… ⌘K" autoComplete="off" style={{ flex:1, background:'transparent', border:'none', outline:'none', fontSize:'13px', color:'var(--text)', minWidth:0 }} />
          {searchQuery
            ? <button type="button" onClick={clearSearch} style={{ background:'transparent', border:'none', cursor:'pointer', color:MUTED, padding:0, display:'flex', alignItems:'center' }}><X size={13} /></button>
            : <span style={{ fontSize:'10px', color:MUTED, background:'var(--surface)', border:'1px solid var(--border)', padding:'1px 5px', borderRadius:'4px', flexShrink:0, whiteSpace:'nowrap', display:'flex', alignItems:'center', gap:'2px' }}><Sparkles size={8} /> AI</span>}
        </div>
      </form>
      {showSearch && (
        <div style={{ position:'absolute', top:'calc(100% + 10px)', left:'50%', transform:'translateX(-50%)', width:'min(540px, 95vw)', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'16px', boxShadow:'var(--shadow3)', overflow:'hidden', zIndex:9999, animation:'ai-panel-in 0.18s cubic-bezier(0.34,1.56,0.64,1)' }}>
          {searchLoading && (
          ...
          {!searchLoading && searchResults && (
            <>
              ...
              {(searchResults.insights?.length ?? 0) > 0 && (
                <div style={{ padding:'10px 16px', borderBottom:'1px solid var(--border)', background:'var(--surface2)' }}>
                  <ul style={{ margin:0, paddingLeft:'18px', fontSize:'12px', color:'var(--text2)', lineHeight:1.5 }}>
                    {searchResults.insights!.map((line, i) => <li key={i} style={{ marginBottom:'4px' }}>{line}</li>)}
                  </ul>
                </div>
              )}
              {(() => {
                ...
                const isStatusChart = (chartData[0] as Record<string,unknown>)?.status != null;
                ...
                return (
                  <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)', background:'var(--surface2)' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'8px' }}>
                      ...
                      <span style={{ fontSize:'11px', fontWeight:700, color:MUTED, textTransform:'uppercase', letterSpacing:'0.05em' }}>{chartLabel}{(searchResults.insight?.period||searchResults.chart?.period) ? ` · ${searchResults.insight?.period||searchResults.chart?.period}` : ''}</span>
                    </div>
                    <div style={{ width:'100%', height:220 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData as object[]} layout="vertical" margin={{ top:4, right:8, left:4, bottom:4 }}>
                          <XAxis type="number" hide />
                          <YAxis type="category" dataKey={xKey} width={140} tick={{ fontSize:11, fill:'var(--text)' }} tickFormatter={(v: string) => v && v.length > 22 ? v.slice(0,20)+'…' : v} />
                          <Tooltip contentStyle={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'8px', fontSize:'12px' }} labelStyle={{ color:'var(--text)' }} formatter={(value: unknown) => [value as React.ReactNode, isStatusChart ? 'Count' : 'Deliveries'] as [React.ReactNode, string]} />
              ...
              {(searchResults.results?.length ?? 0) > 0 && (
                <div style={{ maxHeight:'220px', overflowY:'auto' }}>
                  <div style={{ padding:'9px 16px 4px', fontSize:'11px', fontWeight:700, color:MUTED, textTransform:'uppercase', letterSpacing:'0.05em' }}>Deliveries ({searchResults.totalCount ?? searchResults.results!.length})</div>
                  {searchResults.results!.map(d => {
                    const sc = statusColor(d.status);
                    return (
                      <div key={d.id} onClick={() => handleResultClick(d, 'delivery')} style={{ padding:'10px 16px', cursor:'pointer', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:'12px', transition:'background 0.1s' }} onMouseEnter={e=>(e.currentTarget.style.background='var(--surface2)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                        ...
                        <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 8px', borderRadius:'999px', background:sc.bg, color:sc.color, whiteSpace:'nowrap', textTransform:'capitalize', flexShrink:0 }}>{d.status}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {(searchResults.drivers?.length ?? 0) > 0 && (
                <div style={{ maxHeight:'160px', overflowY:'auto' }}>
                  <div style={{ padding:'9px 16px 4px', fontSize:'11px', fontWeight:700, color:MUTED, textTransform:'uppercase', letterSpacing:'0.05em', display:'flex', alignItems:'center', gap:'6px' }}><Users size={11} /> Drivers ({searchResults.drivers!.length})</div>
                  {searchResults.drivers!.map(d => (
                    <div key={d.id} onClick={() => handleResultClick(d, 'driver')} style={{ padding:'10px 16px', cursor:'pointer', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:'12px', transition:'background 0.1s' }} onMouseEnter={e=>(e.currentTarget.style.background='var(--surface2)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                      <div style={{ width:'32px', height:'32px', borderRadius:'50%', background:'var(--primary)', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:'12px', fontWeight:700, flexShrink:0 }}>{(d.fullName||d.username||'D')[0].toUpperCase()}</div>
                      <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'999px', background: d.active ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.1)', color: d.active ? '#22c55e' : '#ef4444', flexShrink:0 }}>{d.active ? 'Active' : 'Inactive'}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {!searchLoading && !searchResults && (
            <div style={{ padding:'14px' }}>
              ...
              {SEARCH_SUGGESTIONS.map(s => (
                <button key={s} onClick={() => triggerSuggestion(s)} style={{ display:'flex', alignItems:'center', gap:'8px', width:'100%', textAlign:'left', padding:'8px 12px', borderRadius:'8px', background:'transparent', border:'none', cursor:'pointer', fontSize:'13px', color:'var(--text2)', marginBottom:'2px' }} onMouseEnter={e=>(e.currentTarget.style.background='var(--surface2)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                  <Zap size={12} style={{ color:MUTED, flexShrink:0 }} />{s}
                </button>
              ))}
            </div>
          )}
          <div style={{ padding:'8px 16px', borderTop:'1px solid var(--border)', background:'var(--surface2)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            ...
            <span style={{ fontSize:'10px', color:MUTED, display:'flex', alignItems:'center', gap:'3px' }}><Sparkles size={9} /> Electrolux Advanced AI Search</span>
          </div>
        </div>
      )}
    </div>
...
export default function Header({ isAdmin = false }: HeaderProps) {
  ...
  const prevDeliveryIdsRef      = useRef(new Set<string>());
  const prevMsgNotificationsRef = useRef<Notification[]>([]);
  const contactsCacheRef        = useRef<{ data: Map<string, Record<string,unknown>> | null; fetchedAt: number }>({ data: null, fetchedAt: 0 });
  ...
  const [profileData, setProfileData] = useState<ProfileData>({ fullName:'', email:'', phone:'', profilePicture:null, profilePicturePreview:null });
  ...
  useEffect(() => {
    function refresh() {
      ...
      if (u) {
        setProfileData({
          fullName: (u as unknown as Record<string, unknown>).full_name as string || u.username || '',
          email: u.email || '',
          phone: (u as unknown as Record<string, unknown>).phone as string || '',
          profilePicture: null,
          profilePicturePreview: (u as unknown as Record<string, unknown>).profile_picture as string || (u as unknown as Record<string, unknown>).profilePicture as string || null,
        });
      }
    }
  ...
  // Close overlays (search, notifications, profile, mobile nav) on any route change
  useEffect(() => {
    setShowSearch(false);
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'k') { e.preventDefault(); searchInputRef.current?.focus(); setShowSearch(true); }
      if (e.key === 'Escape') { setShowSearch(false); searchInputRef.current?.blur(); }
    };
  ...
  useEffect(() => {
    ...
    const poll = () => { void loadNotifications(); schedule(); };
    const schedule = () => { if (timer) clearTimeout(timer); if (visible) timer = setTimeout(poll, 15000); };
    const onVisibility = () => { visible = !document.hidden; if (visible) { void loadNotifications(); schedule(); } else { if (timer) { clearTimeout(timer); timer = null; } } };
    const onStatusUpdate = (e: Event) => {
      const id = (e as CustomEvent<{ deliveryId?: string }>).detail?.deliveryId;
    ...
    return () => { if (timer) clearTimeout(timer); document.removeEventListener('visibilitychange', onVisibility); window.removeEventListener('deliveryStatusUpdated', onStatusUpdate); };
  ...
  const loadNotifications = async () => {
    try {
      ...
      const cuRaw = cu as unknown as Record<string, unknown>;
      const userRole = ((cuRaw?.account as Record<string, unknown>)?.role as string) || cu?.role || 'driver';
      let unreadCount = 0; let deliveryNotifs: Notification[] = []; let msgNotifs: Notification[] = [];

      if (userRole === 'driver') {
        try {
          const r = await api.get<{ count?: number }>('/messages/driver/notifications/count');
        ...
        try {
          const r = await api.get<Record<string, number>>('/messages/unread');
          const entries = Object.entries(r.data || {}).map(([id, c]) => [id, Number(c)||0] as [string, number]).filter(([,c]) => c > 0);
          ...
          if (entries.length) {
            ...
            if (contactsCacheRef.current.data && age < 5*60*1000) { lookup = contactsCacheRef.current.data; }
            else {
              try {
                const ur = await api.get<{ contacts?: Array<Record<string,unknown>> }>('/messages/contacts');
            ...
            msgNotifs = entries.map(([sid, c]) => {
              ...
              const name = s?.fullName as string || s?.full_name as string || s?.username as string || `User ${String(sid).slice(0,6)}`;
              return { id:`messages-${sid}`, type:'message', count:c, senderId:sid, senderRole:(s?.account as Record<string,unknown>)?.role as string||'user', title:`${c} unread message${c!==1?'s':''} from ${name}`, message:`Open chat with ${name}`, timestamp:new Date(), read:false };
            });
          }
        } catch { /* ignore */ }
      }
      ...
      if (userRole === 'admin' || userRole === 'delivery_team' || userRole === 'logistics_team') {
        try {
          const [ar, or, ur] = await Promise.allSettled([
            ...
            api.get<{ deliveries?: Array<Record<string,unknown>> }>('/admin/notifications/overdue-deliveries'),
            api.get<{ deliveries?: Array<Record<string,unknown>> }>('/admin/notifications/unconfirmed-deliveries'),
          ]);
      ...
      let playBell = false; let newDelCount = 0;
      ...
      if (hasLoadedRef.current) {
        ...
        if (newDelCount > 0) { addToast('Delivery Status Update','delivery',`${newDelCount} delivery update${newDelCount!==1?'s':''}`, 'Click notification bell', 7000); playBell = true; }
      }
      ...
      setNotifications(prev => {
        ...
        return next.sort((a,b) => new Date(b.timestamp).getTime()-new Date(a.timestamp).getTime()).slice(0, 50);
      });
    } catch { /* ignore */ }
  };
  ...
  const handleSearch = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      const { data } = await api.post<SearchResults>('/ai/search', { query: q });
  ...
  const triggerSuggestion = useCallback(async (text: string) => {
    ...
    try {
      const { data } = await api.post<SearchResults>('/ai/search', { query: text });
      setSearchResults(data);
    } catch {
      setSearchResults({ answer:'Search failed. Please try again.', results:[], drivers:[] });
    } finally { setSearchLoading(false); }
  ...
  const handleResultClick = useCallback((result: SearchResult | SearchDriver, type: string) => {
    setShowSearch(false); setSearchQuery(''); setSearchResults(null);
    const userRaw2 = user as unknown as Record<string, unknown>;
    const role = user?.role || ((userRaw2?.account as Record<string, unknown>)?.role as string) || 'driver';
    if (type==='delivery') {
  ...
  const clearSearch = useCallback(() => { setSearchQuery(''); setSearchResults(null); setShowSearch(false); }, []);

  const compressImage = (dataUrl: string, maxSize=400, quality=0.8): Promise<string> => new Promise(res => {
    const img = new Image(); img.crossOrigin='anonymous';
    img.onload = () => {
      const canvas = document.createElement('canvas');
      let { width, height } = img;
      if (width>maxSize||height>maxSize) { if (width>height) { height=Math.round(height*maxSize/width); width=maxSize; } else { width=Math.round(width*maxSize/height); height=maxSize; } }
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setPasswordSuccess(true); setPasswordData({ currentPassword:'', newPassword:'', confirmPassword:'' });
      setTimeout(() => { setShowChangePassword(false); setShowProfileModal(false); void handleLogout(); }, 2000);
    } catch (err: unknown) {
      const e = err as { response?: { data?: { error?: string; message?: string } }; message?: string };
      const msg = e?.response?.data?.error || e?.response?.data?.message || 'Failed to change password';
  ...
  const getInitials = () => {
    const name = (user as unknown as Record<string, unknown>)?.full_name as string || (user as unknown as Record<string, unknown>)?.fullName as string || user?.username || '';
    const parts = name.split(' ').filter(Boolean);
    return parts.length >= 2 ? (parts[0][0]+parts[1][0]).toUpperCase() : (name[0]||'U').toUpperCase();
  };
  const displayName = () => (user as unknown as Record<string, unknown>)?.full_name as string || (user as unknown as Record<string, unknown>)?.fullName as string || user?.username || 'User';
  const userEmail   = () => user?.email || '';
  const userRole    = () => { const r = user?.role||''; return r.charAt(0).toUpperCase()+r.slice(1); };
  const avatarSrc   = () => profileData.profilePicturePreview || (user as unknown as Record<string, unknown>)?.profile_picture as string || (user as unknown as Record<string, unknown>)?.profilePicture as string || null;
  ...
  const isNavActive = (path: string, exact: boolean) => exact ? location.pathname===path : location.pathname.startsWith(path);
  ...
  const toggleTheme = () => {
    ...
    const apply = () => { document.documentElement.classList.remove('light','dark'); document.documentElement.classList.add(next); document.documentElement.style.removeProperty('background-color'); localStorage.setItem('theme', next); setTheme(next); };
    if (typeof (document as Document & { startViewTransition?: (fn: () => void) => void }).startViewTransition === 'function') {
      (document as Document & { startViewTransition: (fn: () => void) => void }).startViewTransition(apply);
    } else { document.documentElement.classList.add('theme-changing'); apply(); setTimeout(() => document.documentElement.classList.remove('theme-changing'), 500); }
  };

  const MUTED   = theme==='dark' ? '#9CA3C4' : '#6b7280';
  const PRIMARY = theme==='dark' ? '#E8EAF6' : '#1A1D3B';
  const iconBtn: React.CSSProperties  = { width:'34px', height:'34px', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'none', cursor:'pointer', color:MUTED, transition:'background 0.15s, color 0.15s', flexShrink:0 };
  const onHover: Partial<CSSStyleDeclaration>  = { background: theme==='dark' ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)', color: PRIMARY };
  const offHover: Partial<CSSStyleDeclaration> = { background:'transparent', color: MUTED };

  const searchBarProps: AISearchBarProps = { outerRef:searchRef, inputRef:searchInputRef, searchQuery, setSearchQuery, searchLoading, searchResults, showSearch, setShowSearch, handleSearch, triggerSuggestion, handleResultClick, clearSearch, theme };

  const NotifPanel = () => {
    const baseStyle: React.CSSProperties = isMobileViewport
  ...
  const UserPanel = () => (
    <div style={{ position:'absolute', right:0, top:'calc(100% + 8px)', width:'240px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', boxShadow:'var(--shadow3)', overflow:'hidden', zIndex:9999, opacity:showDropdown?1:0, transform:showDropdown?'translateY(0) scale(1)':'translateY(-8px) scale(0.95)', transformOrigin:'top right', pointerEvents:showDropdown?'auto':'none', transition:'opacity 0.18s ease, transform 0.22s cubic-bezier(0.34,1.56,0.64,1)' }}>
  ...
  const renderMobileDrawer = (children: React.ReactNode): React.ReactNode => {
    ...
    return (
      <>
        <div className="fixed inset-0 md:hidden" style={{ background:'rgba(0,0,0,0.55)', zIndex:9997 }} onClick={() => setMobileNavOpen(false)} />
        <div className="fixed top-0 left-0 h-full md:hidden flex flex-col animate-slide-in-left" style={{ width:'min(78vw, 290px)', zIndex:9998, background:'var(--bg)', boxShadow:'4px 0 40px rgba(0,0,0,0.35)' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 16px', borderBottom:'1px solid var(--border)' }}>
            ...
            <button onClick={() => setMobileNavOpen(false)} style={{ width:'34px', height:'34px', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', background:'transparent', border:'none', cursor:'pointer', color:MUTED }}><X size={20} /></button>
          </div>
          <div style={{ padding:'12px 14px', borderBottom:'1px solid var(--border)' }}>
            <form onSubmit={(e) => { setMobileNavOpen(false); void handleSearch(e); }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:'10px', padding:'8px 12px' }}>
                ...
                <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search deliveries, drivers…" style={{ flex:1, background:'transparent', border:'none', outline:'none', fontSize:'13px', color:'var(--text)' }} />
              </div>
            </form>
          </div>
          <nav style={{ flex:1, overflowY:'auto', padding:'10px' }}>{children}</nav>
          <div style={{ padding:'14px 16px', borderTop:'1px solid var(--border)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'12px' }}>
              ...
              <div style={{ flex:1, minWidth:0 }}><p style={{ fontSize:'13px', fontWeight:600, color:'var(--text)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{displayName()}</p><p style={{ fontSize:'11px', color:'var(--muted)', textTransform:'capitalize', marginTop:'2px' }}>{userRole()}</p></div>
            </div>
            <button onClick={() => { setMobileNavOpen(false); setShowProfileModal(true); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:'8px', padding:'9px 12px', borderRadius:'8px', background:'var(--surface2)', border:'none', cursor:'pointer', color:'var(--text2)', fontSize:'13px', fontWeight:500, marginBottom:'6px' }}><User size={15} /> Edit Profile</button>
            <button onClick={() => { setMobileNavOpen(false); void handleLogout(); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:'8px', padding:'9px 12px', borderRadius:'8px', background:'rgba(239,68,68,0.1)', border:'none', cursor:'pointer', color:'#ef4444', fontSize:'13px', fontWeight:500 }}><LogOut size={15} /> Sign Out</button>
          </div>
        </div>
      </>
    );
                  ...
                  <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)', background:'var(--surface2)' }}>
                    ...
                    <div style={{ width:'100%', height:220 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData as object[]} layout="vertical" margin={{ top:4, right:8, left:4, bottom:4 }}>
                          <XAxis type="number" hide />
                          <YAxis type="category" dataKey={xKey} width={110} tick={{ fontSize:11, fill:'var(--text)' }} tickFormatter={(v: string) => v && v.length > 16 ? v.slice(0,14)+'…' : v} />
                          <Tooltip contentStyle={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'8px', fontSize:'12px' }} labelStyle={{ color:'var(--text)' }} formatter={(value: unknown) => [value as React.ReactNode, isStatusChart ? 'Count' : 'Deliveries'] as [React.ReactNode, string]} />
                          <Bar dataKey="count" fill="var(--primary)" radius={[0,4,4,0]} minPointSize={6} />
                        </BarChart>
                      </ResponsiveContainer>
  ...
  const pillStyle = (active: boolean): React.CSSProperties => ({
    ...
    boxShadow: active ? 'var(--shadow1)' : 'none', textDecoration:'none', border:'none', cursor:'pointer',
    transition:'background 0.15s, color 0.15s, box-shadow 0.15s', whiteSpace:'nowrap', flexShrink:0,
  });
      ...
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:'16px' }}>
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:'16px', boxShadow:'0 24px 60px rgba(0,0,0,0.5)', width:'100%', maxWidth:'440px', maxHeight:'90vh', overflowY:'auto' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'20px 24px', borderBottom:'1px solid var(--border)', position:'sticky', top:0, background:'var(--bg-card)' }}>
            ...
            <button onClick={() => setShowProfileModal(false)} style={{ background:'transparent', border:'none', cursor:'pointer', color:'var(--text-muted)', padding:'4px' }}><X size={20} /></button>
          </div>
          <div style={{ padding:'24px', display:'flex', flexDirection:'column', gap:'20px' }}>
            ...
            {([['Full Name','text',profileData.fullName,'fullName'],['Email','email',profileData.email,'email'],['Phone','tel',profileData.phone,'phone']] as [string,string,string,keyof ProfileData][]).map(([label,type,val,key]) => (
              <div key={key}>
                <label style={{ display:'block', fontSize:'12px', fontWeight:600, color:'var(--text-secondary)', marginBottom:'6px' }}>{label}</label>
                <input type={type} value={val as string} onChange={e => setProfileData(p=>({...p,[key]:e.target.value}))} style={{ width:'100%', padding:'10px 12px', background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:'10px', fontSize:'14px', color:'var(--text-primary)', outline:'none' }} onFocus={e=>(e.target.style.borderColor='var(--accent)')} onBlur={e=>(e.target.style.borderColor='var(--border)')} />
              </div>
            ))}
            <div>
            ...
            <div style={{ borderTop:'1px solid var(--border)', paddingTop:'16px' }}>
              ...
              {showChangePassword && (
                <div style={{ marginTop:'12px', display:'flex', flexDirection:'column', gap:'12px' }}>
                  {passwordSuccess ? (
                    <div style={{ padding:'12px', background:'rgba(34,197,94,0.1)', border:'1px solid rgba(34,197,94,0.3)', borderRadius:'10px' }}><p style={{ color:'#22c55e', fontSize:'13px', fontWeight:600 }}>✓ Password changed! Redirecting to login…</p></div>
                  ) : (
                    <>
                      ...
                      {([['Current Password','currentPassword'],['New Password','newPassword'],['Confirm New Password','confirmPassword']] as [string, keyof PasswordData][]).map(([lbl,key]) => (
                        <div key={key}>
                          <label style={{ display:'block', fontSize:'12px', fontWeight:600, color:'var(--text-secondary)', marginBottom:'6px' }}>{lbl}</label>
                          <input type="password" value={passwordData[key]} onChange={e=>setPasswordData(p=>({...p,[key]:e.target.value}))} style={{ width:'100%', padding:'10px 12px', background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:'10px', fontSize:'14px', color:'var(--text-primary)', outline:'none' }} onFocus={e=>(e.target.style.borderColor='var(--accent)')} onBlur={e=>(e.target.style.borderColor='var(--border)')} />
                        </div>
                      ))}
                      <button onClick={() => void handleChangePassword()} style={{ padding:'11px', background:'#d97706', color:'white', border:'none', borderRadius:'10px', cursor:'pointer', fontSize:'14px', fontWeight:600 }}>Update Password</button>
                    </>
                  )}
                </div>
              )}
            ...
            <div style={{ display:'flex', gap:'10px', paddingTop:'4px' }}>
              <button onClick={() => setShowProfileModal(false)} style={{ flex:1, padding:'11px', background:'transparent', border:'1px solid var(--border)', borderRadius:'10px', cursor:'pointer', color:'var(--text-secondary)', fontSize:'14px', fontWeight:500 }}>Cancel</button>
              <button onClick={() => void handleSaveProfile()} style={{ flex:1, padding:'11px', background:'var(--accent)', color:'white', border:'none', borderRadius:'10px', cursor:'pointer', fontSize:'14px', fontWeight:600, display:'flex', alignItems:'center', justifyContent:'center', gap:'6px' }}><Save size={14} /> Save Changes</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <>
        <header className="fixed inset-x-0 top-0 z-[900] shrink-0" style={{ background:'var(--bg)', paddingTop:'env(safe-area-inset-top, 0px)' }}>
          <div className="header-inner">
            ...
            <div style={{ flex:1 }} />
            <nav className="hidden md:flex items-center shrink-0" style={{ gap:'2px', marginRight:'12px' }}>
              {ADMIN_NAV.map(item => (
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'2px', flexShrink:0 }}>
              ...
              <div style={{ position:'relative' }} ref={notifRef}>
                <button onClick={() => setShowNotifications(v=>!v)} style={{ ...iconBtn, position:'relative' }} onMouseEnter={e=>Object.assign(e.currentTarget.style, onHover)} onMouseLeave={e=>Object.assign(e.currentTarget.style, offHover)}>
                  <Bell size={17} />
                  {unreadCount>0 && <span style={{ position:'absolute', top:'4px', right:'4px', minWidth:'15px', height:'15px', background:'#ef4444', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'9px', fontWeight:700, color:'white', padding:'0 2px' }}>{unreadCount>9?'9+':unreadCount}</span>}
                </button>
                <NotifPanel />
              </div>
              <div style={{ position:'relative', marginLeft:'6px' }} ref={dropdownRef}>
                <button onClick={() => setShowDropdown(v=>!v)} style={{ display:'flex', alignItems:'center', gap:'9px', padding:'5px 12px 5px 5px', borderRadius:'999px', background:theme==='dark'?'var(--surface2)':'#ffffff', border:'1px solid var(--border)', boxShadow:'var(--shadow1)', cursor:'pointer' }}>
                  <div style={{ width:'30px', height:'30px', borderRadius:'50%', background:'var(--primary)', overflow:'hidden', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:700, fontSize:'12px' }}>{avatarSrc()?<img src={avatarSrc()!} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/>:getInitials()}</div>
                  <div className="hidden sm:block" style={{ textAlign:'left', lineHeight:1 }}><p style={{ fontSize:'13px', fontWeight:600, color:PRIMARY }}>{displayName()}</p><p style={{ fontSize:'11px', color:MUTED, marginTop:'3px', textTransform:'capitalize' }}>{userRole()}</p></div>
