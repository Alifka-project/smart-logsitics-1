  ...
  try {
    ...
    if (plan.intent === INTENTS.NAVIGATION) {
      if (navSuggestions.length > 0) {
        const first = navSuggestions[0] as { label: string; path: string };
    ...
    if (plan.intent === INTENTS.RANKING && plan.dimension === DIMENSIONS.CUSTOMER && userRole === 'admin') {
      ...
      if (rows.length === 0) {
        ...
        const first = rows[0] as { customer: string; deliveries: number };
        ...
        if (rows.length > 1) {
          const rest = (rows as Array<{ customer: string; deliveries: number }>).slice(1, 5).map((r, i) => `${i + 2}. ${r.customer} (${r.deliveries})`).join('; ');
          answer += ` Next: ${rest}.`;
        }
        kpis = { leader: first.customer, leaderCount: first.deliveries };
        table = (rows as Array<{ customer: string; deliveries: number }>).map(r => ({ customer: r.customer, deliveries: r.deliveries }));
        const chartData = (table as Array<{ customer: string; deliveries: number }>).map(r => ({ name: r.customer, count: r.deliveries }));
    ...
    if (plan.intent === INTENTS.RANKING && plan.dimension === DIMENSIONS.PRODUCT && userRole === 'admin') {
      ...
      if (rows.length === 0) {
        ...
        const first = rows[0] as { name: string; deliveries: number };
        const periodStr = period ? `For ${period}, ` : '';
        ...
        table = (rows as Array<{ name: string; deliveries: number }>).map(r => ({ name: r.name, deliveries: r.deliveries }));
        const chartData = (table as Array<{ name: string; deliveries: number }>).map(r => ({ name: r.name, count: r.deliveries }));
    ...
    if (results.length > 0 || drivers.length > 0) {
      ...
      if (navSuggestions.length > 0) {
        answer += ` To explore, try opening "${(navSuggestions[0] as { label: string }).label}".`;
      } else {
...
router.get('/navbar-stats', async (req: Request, res: Response): Promise<void> => {
  try {
    const user = req.user as { id?: string; sub?: string; role?: string } | undefined;
    const userRole = user?.role || 'driver';
    const driverId = user?.sub;
...
router.post('/optimize', async (_req: Request, res: Response): Promise<void> => {
