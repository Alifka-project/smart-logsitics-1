...
>(rows: T[]): Promise<{ attempted: number; succeeded: number; failed: number }> {
  if (!isGeocoderConfigured()) {
    const attempted = rows.filter((r) => r._usedDefaultCoords).length;
    if (attempted > 0) {
      console.warn(
        `[Geocoder] GOOGLE_GEOCODING_KEY not set — ${attempted} rows will keep default coords. Set the env var to enable auto-geocoding.`,
      );
    }
  ...
  console.log(
    `[Geocoder] Batch complete — attempted=${attempted}, succeeded=${succeeded}, failed=${failed}`,
  );
