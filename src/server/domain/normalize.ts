...
function customerNormalized(value: unknown = ''): string {
  const s = normalizeForKey(value);
  return s || 'Unknown';
}

function productNormalized(value: unknown = ''): string {
  const s = normalizeForKey(value);
 ...
 */
function parseProductNames(itemsField: unknown): string[] {
  const names: string[] = [];
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      (parsed as Record<string, unknown>[]).forEach(p => {
