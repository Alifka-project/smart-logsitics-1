...
export default function AdminDriverTrackingPage(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const handleVisChange = (): void => {
      if (!document.hidden) void loadDriverTracking();
    };
    document.addEventListener('visibilitychange', handleVisChange);

    const handleDeliveriesUpdated = (): void => { void loadDriverTracking(); };
    const handleDeliveryStatusUpdated = (): void => { void loadDriverTracking(); };
  ...
  const loadDriverTracking = async (): Promise<void> => {
    try {
      const response = await api.get('/admin/tracking/drivers');
      setDrivers((response.data as { drivers?: Driver[] }).drivers || []);
      setLastUpdate(new Date());
    } catch (err: unknown) {
      console.error('Error loading driver tracking:', err);
    } finally {
      setLoading(false);
    }
  };
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      {location ? (
                        <div>
                          ...
                          {location.speed && (
                            <div className="text-xs text-gray-400 dark:text-gray-500">{((location.speed as number) * 3.6).toFixed(0)} km/h</div>
                          )}
                        </div>
                      ) : (
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      {location?.timestamp 
                        ...
                        : driver.tracking?.lastUpdate
                          ? new Date(driver.tracking.lastUpdate as string).toLocaleTimeString()
                          : 'N/A'
                      }
                    </td>
                  </tr>
