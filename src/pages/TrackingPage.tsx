...
const STATUS_MAP: Record<string, StatusInfo> = {
  ...
  'pickup-confirmed': { icon: Package, text: 'Pickup Confirmed', color: 'text-teal-600' },
...
export default function TrackingPage() {
  ...
  if (confirmed) {
    const statusInfo = getStatusInfo(delivery?.['status'] as string);
        ...
        <div className="pp-dash-card shadow-2xl max-w-md w-full p-8 text-center">
          ...
          {delivery && (
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              ...
              <div className="text-sm text-gray-600">Delivery ID: {deliveryId}</div>
            </div>
          )}
          <button
