            ...
            <ResponsiveContainer width="100%" height={300}>
              <ComposedChart data={heroChartData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                ...
                <Tooltip 
                  ...
                  formatter={(val: unknown, name: string) => (name === 'Success Rate %' ? [`${val as string}%`, name] : [val as React.ReactNode, name]) as [React.ReactNode, string]}
                />
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                        {(delivery.created_at || delivery.createdAt)
                          ? new Date((delivery.created_at || delivery.createdAt) as string).toLocaleDateString('en-GB')
                          : 'N/A'}
                      </td>
                        ...
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            onChange={async (e: React.ChangeEvent<HTMLSelectElement>) => {
                              ...
                              } catch (err: unknown) { console.error('Status update error:', err); }
                            }}
                  ...
                  <MapContainer center={[25.2, 55.27]} zoom={11} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false}>
                    ...
                    {mapPoints.map((r, i) => {
                      ...
                      return (
                        <CircleMarker
                          key={r.area || i}
                          center={r.coords as [number, number]}
                          radius={radius}
                ...
                {driversData.length > 0 ? driversData.map(driver => {
                  ...
                  return (
                    <tr key={driver.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                        {lastSeen
                          ? new Date(lastSeen as string).toLocaleString('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })
                          : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <button
