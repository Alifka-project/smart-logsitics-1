...
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      const { driver, clientKey, csrfToken, accessToken } = res.data as {
        driver: import('../types').AuthUser;
        clientKey: string;
        csrfToken: string;
        accessToken: string;
      };
  ...
  const loginFormContent = (
    <>
      ...
      <p className="mt-4 text-center text-[11px] text-gray-400 leading-relaxed">
        🔒 This portal is for{' '}
        <span className="font-semibold text-gray-500">Electrolux internal use only</span>.<br />
