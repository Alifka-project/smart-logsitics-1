...
router.post('/osrm', auth_js_1.authenticate, (0, auth_js_1.requireRole)('admin'), async (req, res) => {
    ...
    try {
        ...
        if (!trip?.legs?.length)
            throw new Error('Empty Valhalla response');
        let allCoords = [];
        ...
        if (!allCoords.length)
            throw new Error('No coordinates from Valhalla');
        ...
        console.warn('[Routing] Valhalla failed, trying OSRM:', valhallaErr.message);
    }
