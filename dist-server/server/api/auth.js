...
const { hashPassword, comparePassword, generateAccessToken, createLoginSession, clearLoginSession, refreshAccessToken, authenticate, destroyUserSessions } = require('../auth');
...
router.patch('/profile', authenticate, async (req, res) => {
    try {
        ...
        if (fullName !== undefined)
            updateData.fullName = String(fullName).trim() || null;
        if (email !== undefined)
            updateData.email = String(email).trim() || null;
        if (phone !== undefined)
            updateData.phone = String(phone).trim() || null;
        if (profilePicture !== undefined)
            updateData.profilePicture = profilePicture && String(profilePicture).trim() ? String(profilePicture).trim() : null;
...
function generateTemporaryPassword() {
    ...
    function pick(chars) {
        return chars[crypto_1.default.randomInt(0, chars.length)];
    }
    const passwordChars = [
        pick(upper),
        pick(lower),
