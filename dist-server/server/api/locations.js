...
async function checkDriverArrival(driverId, driverLat, driverLng) {
    try {
        ...
        for (const assignment of assignments) {
            ...
            if (doneStatuses.includes((delivery.status || '').toLowerCase()))
                continue;
            ...
            const recentNotif = await prisma_js_1.default.adminNotification.findFirst({
                where: {
                    type: 'driver_arrived',
                    createdAt: { gt: cutoff },
            ...
            await prisma_js_1.default.adminNotification.create({
                data: {
                    type: 'driver_arrived',
...
router.post('/me/location', auth_js_1.authenticate, async (req, res) => {
    ...
    try {
        const created = await prisma_js_1.default.liveLocation.create({
            data: {
                driverId,
                latitude: lat,
                longitude: lng,
        ...
        setTimeout(async () => {
            try {
                await prisma_js_1.default.liveLocation.deleteMany({
                    where: {
                        driverId,
        ...
        cache_js_1.default.invalidatePrefix('tracking:');
        res.json({
            ok: true,
            location: {
...
router.get('/me/live', auth_js_1.authenticate, async (req, res) => {
    const driverId = getAuthenticatedDriverId(req);
    if (!driverId) {
        res.status(401).json({ error: 'unauthorized' });
        return;
    ...
    try {
        const latest = await prisma_js_1.default.liveLocation.findFirst({
            where: { driverId },
 ...
 */
router.get('/notifications/count', auth_js_1.authenticate, async (req, res) => {
    try {
        const driverId = req.user?.sub;
        if (!driverId) {
            res.status(401).json({ error: 'Unauthorized' });
