...
exports.normalizeForKey = normalizeForKey;
exports.customerNormalized = customerNormalized;
exports.productNormalized = productNormalized;
exports.parseProductNames = parseProductNames;
/**
...
function parseProductNames(itemsField) {
    const names = [];
    if (itemsField == null || String(itemsField).trim() === '')
        return names;
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            parsed.forEach(p => {
                ...
                if (d)
                    names.push(String(d).replace(/\s*x\s*\d+\s*$/i, '').trim());
            });
