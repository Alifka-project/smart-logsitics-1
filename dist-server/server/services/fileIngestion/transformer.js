...
exports.detectDataFormat = detectDataFormat;
exports.transformGenericData = transformGenericData;
/**
 ...
 */
function looksLikePurchaseOrderValue(raw) {
    const v = raw.trim();
    if (!v || v.length > 120)
        return false;
    const lower = v.toLowerCase();
    if (NON_PO_VALUE.has(lower))
        return false;
...
const FUZZY_PO_COLUMN_BLOCK = /status|reason|block|category|indicator|date|time|priority|qty|quantity|amount|line\s*item|item\s*no|net\s*val|gross|weight|volume|route|plant|storage|ship\s*point|picking|loading|partner|incoterms|description|material|text|note|comment/i;
function extractPONumber(row) {
    ...
    for (const [colName, value] of Object.entries(row)) {
        ...
        const poish = lowerCol.includes('po') ||
    ...
    for (const [colName, value] of Object.entries(row)) {
        ...
        if (!mentionsOrderOrDelivery || mentionsPo)
            continue;
        if (FUZZY_PO_COLUMN_BLOCK.test(colName))
            continue;
        if (!looksLikePurchaseOrderValue(val))
            continue;
 ...
 */
function parseGoodsMovementDate(row) {
    if (!row)
        return null;
    const candidates = [
...
function convertToIsoDate(val) {
    ...
    if (typeof val === 'number') {
        if (val < 1000 || val > 200000)
            return null; // Not a plausible date serial
...
function transformERPData(data) {
    return data.map((row, index) => {
        ...
        const shipToVal = (row['Ship-to Party'] ||
            row['Ship To Party'] ||
            row['Ship to Party'] ||
            row['Ship to party'] ||
            row['Ship-to party'] ||
        ...
        const address = [row['Ship to Street'], row['Ship-to Street'], row['Street'], row['Address'], row['City'], row['Postal code']]
            .filter(Boolean)
            .join(', ') ||
        ...
        const phone = row['Telephone1'] ||
            ...
            row['No. HP'] ||
            '';
        const items = [row['Description'], row['Material']].filter(Boolean).join(' - ') || 'Items not specified';
        ...
        const combinedKey = combinedCandidates.find((c) => c in row) ||
        ...
        const normalizedPhone = (0, phoneUtils_1.normalizeUAEPhone)(rawPhone) || rawPhone;
        ...
        return {
            ...
            _deliveryNumber: (row['Delivery number'] ||
                row['Delivery Number'] ||
                row['DeliveryNumber'] ||
                row['Delivery No'] ||
                row['Del. No'] ||
...
function detectDataFormat(data) {
    ...
    const hasSimplifiedFormat = 'customer' in firstRow &&
        'address' in firstRow &&
        'lat' in firstRow &&
        'lng' in firstRow &&
        'phone' in firstRow &&
    ...
    const hasDeliveryOrSalesKey = 'Delivery number' in firstRow ||
        'Delivery Number' in firstRow ||
        'DeliveryNumber' in firstRow ||
        'Sales Document' in firstRow;
    const hasErpCustomerKey = 'Ship-to Party' in firstRow ||
    ...
    const hasPhoneColumn = keys.some((k) => k.toLowerCase().includes('phone') || k.toLowerCase().includes('telephone') || k.toLowerCase().includes('mobile'));
    if (hasCityColumn && (hasAddressColumn || hasPhoneColumn)) {
        return { format: 'generic', transform: transformGenericData };
    }
    return { format: 'unknown', transform: null };
}
function transformGenericData(data) {
    return data.map((row, index) => {
        ...
        const itemsKey = keys.find((k) => k.toLowerCase().includes('item') ||
        ...
        const deliveryNumberKey = keys.find((k) => {
            ...
            if (l.includes('delivery') && l.includes('number'))
                return true;
            if (l === 'deliverynumber')
                return true;
            if (l === 'dn')
                return true;
            if (/^del\.?\s*no\.?$/i.test(k.trim()))
                return true;
            return false;
        });
        const customer = (customerKey ? row[customerKey] : null) || `Customer ${index + 1}`;
        const address = [addressKey ? row[addressKey] : null, cityKey ? row[cityKey] : null]
            .filter(Boolean)
