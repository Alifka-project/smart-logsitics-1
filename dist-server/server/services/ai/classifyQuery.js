...
exports.DIMENSIONS = exports.ENTITIES = exports.INTENTS = void 0;
...
function classifyQuery(rawQuery) {
    ...
    const dateRange = (0, dateFilters_1.inferDateRangeFromQuery)(q);
    // --- Navigation ---
    ...
    const isCount = COUNT_TRIGGERS.some(t => q.includes(t)) ||
        /^(how many|number of|total)\s+(pending|delivered|cancelled|in transit|orders?|deliveries?)/i.test(q);
    const statusForCount = (0, statusMap_1.intentToStatus)(q);
    if (isCount) {
        const countDateRange = dateRange || (q.includes('today') ? (0, dateFilters_1.inferDateRangeFromQuery)('today') : null);
        return {
    ...
    const hasRanking = /\b(top|best|leading|biggest|highest|most)\s*(\d*)\s*(customer|client|product|item)/i.test(q) ||
    ...
    if (hasRanking) {
        ...
        const isProduct = PRODUCT_SYNONYMS.some(s => q.includes(s)) ||
        ...
        if (isProduct) {
            return {
                ...
                filters: { status: null, dateRange: dateRange || (0, dateFilters_1.inferDateRangeFromQuery)('this month') },
                topN,
                rawQuery: rawQuery.trim(),
            };
        }
    ...
    if (/\b(trend|over time|by (day|week|month)|delivered vs|vs cancelled)\b/i.test(q) || (q.includes('last') && (q.includes('day') || q.includes('month')) && (q.includes('delivered') || q.includes('cancelled')))) {
        return {
            ...
            metric: 'count',
            dimension: DIMENSIONS.STATUS,
            filters: { status: null, dateRange: dateRange || (0, dateFilters_1.inferDateRangeFromQuery)('last 30 days') },
            topN: null,
            rawQuery: rawQuery.trim(),
        };
    }
    ...
    return {
        ...
        metric: null,
        dimension: null,
        filters: { status: (0, statusMap_1.intentToStatus)(q) || null, dateRange },
        topN: null,
        rawQuery: rawQuery.trim(),
    };
}
