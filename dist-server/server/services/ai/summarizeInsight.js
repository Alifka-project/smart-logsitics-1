...
exports.summarizeInsight = summarizeInsight;
async function summarizeInsight(payload) {
    const { intent, plan, data, answer } = payload;
    const key = process.env.OPENAI_API_KEY;
    if (!key || !data)
        return [];
    const dataStr = JSON.stringify(data);
    const systemContent = 'You are an analytics summarizer for a logistics system. You receive structured data and a short answer. ' +
