...
exports.normalizeSapDeliveryForTracking = normalizeSapDeliveryForTracking;
 ...
 */
function normalizeSapDeliveryForTracking(raw) {
    const id = firstStr(raw.id, raw.ID, raw.DeliveryDocument, raw.DeliveryID);
    if (!id)
        return raw;
    const customer = firstStr(raw.customer, raw.CustomerName, raw.customerName, raw.Name, raw.CompanyName, raw.ShipToPartyName, raw.SoldToPartyName) ?? null;
    const phone = firstStr(raw.phone, raw.Phone, raw.MobilePhone, raw.Telephone, raw.Mobile, raw.customerPhone, raw.CustomerPhone) ?? null;
    const address = firstStr(raw.address, raw.Address, raw.deliveryAddress, raw.DeliveryAddress, raw.Street, raw.ShipToAddress) ?? null;
    const poNumber = firstStr(raw.poNumber, raw.PONumber, raw.PurchaseOrder, raw.CustomerPONumber, raw['Cust. PO Number'], raw['PO Number']) ?? null;
    const deliveryNumber = firstStr(raw.deliveryNumber, raw.DeliveryNumber, raw['Delivery number'], raw.DeliveryDocument, raw.Delivery) ?? null;
    ...
    const confirmedDeliveryDate = parseDateField(raw.confirmedDeliveryDate) ??
        parseDateField(raw.CustomerRequestedDate) ??
        parseDateField(raw.RequestedDeliveryDate) ??
        parseDateField(raw['Delivery Date']);
    const goodsMovementDate = parseDateField(raw.goodsMovementDate) ??
