...
--
-- NOTE: No backfill is performed intentionally.
-- PostgreSQL UNIQUE constraints allow multiple NULL values, so existing
-- records safely keep delivery_number = NULL until re-uploaded via the
-- dedup service (which sets and validates uniqueness correctly).
